...
function updateGroupsCard() {
	if (!groupsPreviewList || !groupsCardEmpty) {
		return;
	}
	...
	const groups = getStoredArray(GROUPS_KEY)
		.map((group, index) => {
			...
			return {
				...
				name: normalizedName,
				members,
				sortTimestamp: getGroupSortTimestamp(group, index)
			};
		})
	...
	groups.forEach((group) => {
		...
		meta.textContent = group.members.length === 1
			? "1 membro"
...
function syncDecisionTargetsPreserveSelection() {
	if (!decisionGroupSelect || !decisionFriendsSelect) {
		return;
	}

	const previousGroupId = decisionGroupSelect.value;
	const previousFriendIds = Array.from(decisionFriendsSelect.selectedOptions)
	...
	const hasGroupOption = Array.from(decisionGroupSelect.options).some((option) => option.value === previousGroupId);
	if (hasGroupOption) {
		decisionGroupSelect.value = previousGroupId;
	}

	populateFriendsOptions(groups, friends);

	Array.from(decisionFriendsSelect.options).forEach((option) => {
		option.selected = previousFriendIds.includes(option.value);
	});
}

function syncTargetsIfModalOpen() {
	if (!decisionModalOverlay || decisionModalOverlay.hidden) {
		return;
	}
...
function redirectToGroupsCreate() {
	window.location.href = "./groups.html#create-group";
}
...
function handleCreateDecision() {
	...
	const groups = getStoredGroups();
	const friends = getStoredFriends();
	const selectedGroup = decisionGroupSelect && decisionGroupSelect.value
		? groups.find((group) => group.id === decisionGroupSelect.value) || null
		: null;
	const selectedFriendIds = decisionFriendsSelect
		? Array.from(decisionFriendsSelect.selectedOptions).map((option) => option.value).filter((value) => value)
		: [];
	...
	const decision = {
		...
		targetFriends: selectedFriends.map((friend) => ({ id: friend.id, name: friend.name })),
...
if (groupsCreateButton) {
	groupsCreateButton.addEventListener("click", redirectToGroupsCreate);
}
...
if (decisionGroupSelect) {
	decisionGroupSelect.addEventListener("change", () => {
		populateFriendsOptions(getStoredGroups(), getStoredFriends());
	});
}

window.addEventListener("storage", (event) => {
	if (!event.key || event.key === GROUPS_KEY || event.key === FRIENDS_KEY) {
		syncTargetsIfModalOpen();
		updateGroupsCard();
	}
});
