...
function renderActivity(users, decisions) {
    ...
    users.slice(-2).reverse().forEach((u) => {
        items.push({ initials: initials(u.name), user: u.name, action: "registou-se na plataforma" });
    });

    decisions.slice(-2).reverse().forEach((d) => {
        items.push({ initials: "🗳️", user: "Sistema", action: `criou a decisão <strong>"${d.title}"</strong>` });
    });
    ...
    tbody.innerHTML = slice.length === 0
        ...
        : slice.map((u) => `
            <tr>
                ...
                <td>${u.email}</td>
                <td><span class="badge ${u.is_admin ? 'badge-admin' : 'badge-user'}">${u.is_admin ? 'Admin' : 'Utilizador'}</span></td>
...
window.saveUser = async function (id) {
    ...
    try {
        const updated = await api.updateUser(id, { name, email, password, profile_picture: u.profile_picture || null });
...
async function loadDecisions() {
    try {
        decisionsData = await api.getDecisions();
        renderDecisions();
    ...
    grid.innerHTML = filtered.length === 0
        ...
        : filtered.map((d) => `
            <div class="decision-card">
                <div class="decision-card-header"><span>ID: ${d.vote_id}</span></div>
...
async function loadGroups() {
    try {
        groupsData = await api.getUserGroups();
        renderGroups();
...
function renderGroups() {
    ...
    if (groupsData.length === 0) {
        card.innerHTML = `<p class="empty-state">Nenhum grupo registado ainda.</p>`;
        return;
    }

    card.innerHTML = `
        <div class="card-header"><h2 class="card-title">Todos os grupos</h2></div>
...
window.addGroup = async function () {
    ...
    try {
        const created = await api.createUserGroup(name);
